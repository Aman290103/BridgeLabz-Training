...
namespace UnitTestingPractice.MSTests{
    ...
    public class UserRegistrationMSTest{
        ...
        public void RegisterUser_ValidInput_ReturnsTrue(){
            // Act
            bool result = registration.RegisterUser("lavan","lavan@example.com","Password123");
            // Assert
            Assert.IsTrue(result);
        }
        [TestMethod]
        public void RegisterUser_InvalidUsername_ThrowsException(){
            Assert.ThrowsException<ArgumentException>(() =>registration.RegisterUser("", "test@mail.com", "Password123"));
        }
        [TestMethod]
        public void RegisterUser_InvalidEmail_ThrowsException(){
            Assert.ThrowsException<ArgumentException>(() =>registration.RegisterUser("user", "invalidEmail", "Password123"));
        }
        [TestMethod]
        public void RegisterUser_InvalidPassword_ThrowsException(){
            Assert.ThrowsException<ArgumentException>(() =>registration.RegisterUser("user", "test@mail.com", "123"));
        }
    }
}